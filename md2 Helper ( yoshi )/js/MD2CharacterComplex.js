...
THREE.MD2CharacterComplex = function () {
	...
	this.myUpdate = function ( delta, velocity ) {

		//if ( this.controls ) this.updateMovementModel( delta );
		this.myUpdateMovementModel (delta, velocity)
		
		if ( this.animations ) {

			this.myUpdateBehaviors( delta, velocity );
	...
	this.myUpdateBehaviors = function ( delta, velocity ) {
		moveAnimation = this.animations ["move"]
		idleAnimation = this.animations ["idle"]
...
*/
	...
	this.myUpdateMovementModel = function ( delta, velocity ) {
		
		// speed clamping
		let speed = velocity.length();
		let vel = velocity.clone().multiplyScalar ( THREE.Math.clamp (speed, 0, this.maxSpeed) );
		 
		// speed decay
		// [done in agent level]
		
		// displacement
		this.root.position.add ( vel.clone().multiplyScalar (delta) );
		
		// steering
		// update orientation only if speed is significant
		if (speed > 0.1)  
			this.root.rotation.y = Math.atan2 (- vel.z, vel.x) + Math.PI/2;
	};
